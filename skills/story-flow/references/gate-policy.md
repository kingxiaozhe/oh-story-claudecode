...
- **全书级**：`设定/门禁配置.json`（初始化 Step 4 生成），存对话占比区间、各容差、超长段阈值，以及 v0.2 新增的 `cold_read_mode` 与 `author_checkpoint_every`。改全书阈值只改这一个文件。
...
| 层 | 工具/方式 | 检查什么 |
|----|-----------|----------|
| ① 确定性 | check_gate.py + linter | 字数区间、对话占比、超长段落（advisory）、匀速节拍哨兵（指纹 `sent_len_uniform_run_max` ≥5 → advisory「机器节拍」，humanize 信号B本地化）；AI 风格指纹（blocking 级） |
| ② 语义自审 | 调度员对照控制卡与正典 | 禁止事项未违反、必埋伏笔已埋、出场人物未越界、钩子按卡兑现、与 `追踪/` 无事实冲突、细纲情节点未漏、**声口核对（逐角色抽检+互换测试，见 N4）**；通用维度（一致性/对话质量）的判据向 story-review 的内置审查基准对齐，不另立标准 |
| ③ 冷读 | 冷读代理（见 cold-read-review.md） | 追读意愿、记忆点、跳读点；与控制卡承诺情绪对账 |

**冷读抽检（v0.2 成本旋钮）**：`cold_read_mode` = `"every"`（默认，每章冷读）或 `"sampled"`（高压章/伏笔埋设与回收章/人物首登场与退场章/卷末章**必检**，其余章抽检 1/2；试点数据显示冷读连续 4/5 时抽检的质量风险可控）。被抽过的章在门禁产物记 `cold_read:sampled_skip`。
...
| **advisory** | linter advisory；冷读 3 分；超长段落；匀速节拍连跑 ≥5 | 记入门禁产物放行，不回炉；聚集时交 N6 |
