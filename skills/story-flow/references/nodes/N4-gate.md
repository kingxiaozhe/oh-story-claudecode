...
2. **② 语义对卡审查**：v0.2 起优先派发**对卡审查子代理**（输入=控制卡+正文+`追踪/上下文.md` 最新两节+细纲+人物卡声口字段；输出=固定结构：禁止事项逐条✓✗/必埋伏笔/出场越界/钩子兑现/情节点完整/**声口核对**/事实冲突清单），调度员只读结论做裁决，**不通读正文**——这是调度员上下文不随章数膨胀的关键。代理不可用时降级为调度员亲审（solo 模式先做③再做②，先冷读后对卡），并接受上下文成本。
   **声口核对（v0.2.1 新增维度）**：每个有台词角色抽 2-3 句对照人物卡声口字段核验；执行"互换测试"——任意两角色的台词互换后若不穿帮即判声口趋同。单角色走音=blocking（修台词，属语义轮次账）；两角色以上系统性趋同=blocking 且根因升级（查声口速记是否缺失/人物卡是否欠账）。
