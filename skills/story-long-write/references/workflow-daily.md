...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **2.5 写正文（段落级声口优化循环——唯一写法；`设定/声口.md` 为写作硬闸）**：**无 `设定/声口.md`（或无实质内容）则一个字的正文都不写**——停止流程，引导作者走 声口库/README.md「来源仪式」：推荐 1-3 本同频道同赛道参考书 → 作者选定母本 → 炼制 → 绑定（**新书旧书同流程**，旧稿原文只作分寸校准不作主来源），作者认可绑定后才恢复写作。声口文件就绪后加载 `references/voice-loop.md`，按其 §2 逐段执行"写草稿段 → 按 `设定/声口.md` 优化该段 → 段级自检（防雪球）→ 承接下一段"，周而复始直到整章写完。**红线：每段只锚 `设定/声口.md`，绝不模仿上一段刚写完的成品风格。**
   - 写正文 → **字数验证（优先 Python 字符统计，`wc -m` 仅作 Unix 备选，< 目标90%则对照情节点字数预算定位欠账密点、一次性重写到配额，不挤牙膏反复回炉；> 章目标×1.1 则压过场/合并疏点收敛；90% 是放行下限非目标，理想落在 [章目标, 章目标×1.1]）** → 检查钩子/爽点 → **正文元信息扫描** → 禁用词扫描 → **声口模式下：项目若有 `追踪/gate.py` 则跑该书门禁（逗号密度等硬红线全绿才放行，防逐段滚雪球）**
