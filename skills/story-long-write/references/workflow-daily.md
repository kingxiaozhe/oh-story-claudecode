...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **2.5 写正文（段落级优化循环——唯一写法，整章一次性输出已废除）**：加载 `references/voice-loop.md`，按其 §2 逐段执行"写草稿段 → 按锚点文件优化该段 → 段级自检（防雪球）→ 承接下一段"，周而复始直到整章写完。锚点文件按序取第一个存在的并全章锁定：`设定/声口.md`（首选）→ `设定/文风.md` → 对标文风基线。**红线：每段只锚固定锚点文件，绝不模仿上一段刚写完的成品风格。** 无 `设定/声口.md` 的项目建议尽早从仓库级 `声口库/` 实例化（见 声口库/README.md）。
