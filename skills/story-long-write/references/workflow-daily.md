...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **2.5 写正文（段落级声口优化循环——唯一写法；`设定/声口.md` 为写作硬闸）**：**无 `设定/声口.md`（或无实质内容）则一个字的正文都不写**——停止流程，引导作者从仓库级 `声口库/` 实例化（新书选条目，存量书从已发布原文蒸馏，流程见 声口库/README.md），作者认可后才恢复写作。声口文件就绪后加载 `references/voice-loop.md`，按其 §2 逐段执行"写草稿段 → 按 `设定/声口.md` 优化该段 → 段级自检（防雪球）→ 承接下一段"，周而复始直到整章写完。**红线：每段只锚 `设定/声口.md`，绝不模仿上一段刚写完的成品风格。**
