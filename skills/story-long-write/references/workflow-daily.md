...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **2.5 写正文（声口模式判定）**：
     - **有 `设定/声口.md`（含实质内容）→ 段落级声口优化循环**：**不整章一次性输出**，加载 `references/voice-loop.md`，按其 §2 逐段执行"写草稿段 → 按 `设定/声口.md` 优化该段 → 段级自检（防雪球）→ 承接下一段"，周而复始直到整章写完。**红线：每段只锚 `设定/声口.md`，绝不模仿上一段刚写完的成品风格。**
     - **无 `设定/声口.md` → 整章写法**（原流程）。
   - 写正文 → **字数验证（优先 Python 字符统计，`wc -m` 仅作 Unix 备选，< 目标90%则对照情节点字数预算定位欠账密点、一次性重写到配额，不挤牙膏反复回炉；> 章目标×1.1 则压过场/合并疏点收敛；90% 是放行下限非目标，理想落在 [章目标, 章目标×1.1]）** → 检查钩子/爽点 → **正文元信息扫描** → 禁用词扫描 → **声口模式下：跑 `python3 追踪/gate.py 正文/第{N}章-*.md`，逗号密度 ≤16% 等硬红线全绿才放行（防逐段滚雪球）**
