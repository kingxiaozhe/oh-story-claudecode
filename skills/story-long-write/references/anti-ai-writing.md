...
**段末同理（金句楼梯）**：段落结尾频繁落哲理/定性短句（"这就是人心。""说到底，还是钱的事。"）是同一种病的段级形态——单句看是亮点，成串就是 AI 楼梯。金句式段尾全章 ≤1，且不许连续两段都用金句收尾；多余的改成动作、物件或对话收段。
...
**反向同病：匀速节拍（等长中句连跑）**。电报体是"全压短"，匀速节拍是"全一样"——连续四五句都是 10-20 字的中等句，朗读起来像节拍器，是 AI 最常见的节奏指纹（人写作天然长短错落：短句砸重拍，长句铺呼吸）。判法：连读时数相邻句字数，连续 ≥4 句几乎等长（差 ≤3 字）就中招；story-flow 项目可直接看 check_gate.py 指纹的 `sent_len_uniform_run_max`（≥5 记 advisory）。修法：不是逐句凑字数，而是把其中一句砍成 ≤6 字的重拍短句、或把相邻两句并成一句 25 字以上的自然长句，让长短重新错开——优先在情绪/动作峰值处砍短，在铺垫过渡处并长。
...
- [ ] **句长有落差**：无连续 ≥4 句几乎等长（差≤3字）的匀速节拍；长短句自然错落，重拍处有 ≤6 字短句（见模式 9「反向同病：匀速节拍」）
- [ ] **段末无金句楼梯**：金句式段尾全章 ≤1 且不连续两段（见「章末总结体」段末条款）
