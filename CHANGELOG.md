...
- **story-flow v0.2.6 去AI味防线校准（对照 harshaneel/humanize）**：逐条对照 humanize 九杠杆与 ai-check 九检测信号后，吸收两个真实缺口——①**匀速节拍哨兵**（burstiness deficit 本地化，AI 最常见的节奏指纹：连续多句等长的"节拍器腔"）：check_gate.py 指纹新增 `sent_len_uniform_run_max`（等长中长句最大连跑，相邻差≤3字算等长，<8字短句破节拍重置）与 `short_sent_ratio_pct`，连跑 ≥5 记 advisory「机器节拍」（gate-policy 分级表同步）；anti-ai-writing.md 模式 9 新增「反向同病：匀速节拍」+ 自检清单两项（5 份拷贝字节同步）；voice-loop 段级自检加句长匀速检查、派发模板文风基线加长短错落硬要求。②**段末金句楼梯**：章末总结体扩展到段级——金句式段尾全章 ≤1 且不许连续两段。其余七杠杆经查已有等价或更严实现（破折号全禁、"不是A是B"blocking、三连排比/解释腔/意义膨胀/书面连词/章末总结体、声口+文风双层即其 writer-profile distillation 的强化版），未重复引入；英文特有项（分号/卷引号/缩写语域）不适用中文网文，明确不采。
- **story-flow v0.2.5 声口来源仪式**：把"声口文件从哪来"固化为两场景强制前置——新书定主题后、旧书续写前，一律执行「**推荐 1-3 本同频道同赛道参考书 → 作者选定母本（1 主可加 1 辅，可全否自选）→ 炼制（优先级：声口库已校准条目 → 作者提供母本样本拆条目回流入库 → 凭认知起草并标注待替换）→ 绑定**」；绑定=文件头登记 绑定书名/来源母本/实例化日期/作者认可，全书锁定，换声口走变更模式且须作者亲自确认。**旧书与新书同一套流程**，已发布原文只作分寸校准、不作声口主来源；绑定完成前一个字新正文都不写。落点：`声口库/README.md` 新增「来源仪式」节（权威流程），spec-generation Step 1 与存量书接管（新增第 4 步硬闸）、N3 步骤 0、voice-loop 来源条款、workflow-daily 2.5、story-flow 铁律 8 六处同步改口（原"存量书从已发布原文蒸馏"表述废除）。
- **story-flow v0.2.4 声口硬闸**：`设定/声口.md` 从"首选锚点"升级为**写作前置硬性要求**——无声口文件（或无实质内容）一个字的正文都不写：N3 新增第 0 步硬闸（流水线停 `paused_for_human` 等作者定嗓音）、写手派发模板开工前置检查（缺失即 `blocked` 拒写）、workflow-daily 2.5 与 voice-loop 触发条款同步；锚点降级链（文风.md/文风基线兜底）废除，优化锚点=`设定/声口.md` 唯一；初始化清点将声口实例化列为必备完成项（新书从声口库选条目、存量书从原文蒸馏，均须作者认可）。
- **story-flow v0.2.3 整章写法废除**：段落级优化循环升格为**唯一写作程序**（story-flow 铁律 8 + workflow-daily 2.5 + voice-loop 触发条款三处同步）——任何情况下禁止整章一次性输出；优化锚点按降级链取第一个存在的文件并全章锁定（`设定/声口.md` → `设定/文风.md` → 对标/派发文风基线），无声口文件不再是整章写法的理由，仅意味着锚点降级。
- **story-flow v0.2.2 段落级声口循环接入**：合入 novel-continuation 会话的声口引擎并打通流水线——①`声口库/`（仓库级赛道声口模板库，含通用内核与女频古言两个已校准条目）与 `voice-loop.md`（段落级声口优化循环：写一段→按 `设定/声口.md` 优化一段→防雪球自检→下一段，红线=永不模仿上一段成品）进入 main；②story-long-write workflow-daily 2.5 增声口模式判定路由；③story-flow 写手派发模板新增"写作程序"段——项目有 `设定/声口.md` 时**强制段落级循环、禁止整章一次性输出**；④声音体系三层权威统一声明（声口.md=作者叙述声口 / 文风.md=全书文风基线 / 人物卡=角色台词声口）；⑤check_gate 指纹新增逗号密度哨兵（滚雪球乱码早期指标）。
- **story-flow v0.2.1 声口优化全链路**：角色声口从"写作层软约束"升级为流水线一等公民——①派发指令新增"声口速记"段（每个有台词角色：语言签名/口头禅/禁语，从人物卡派生，缺卡先补）；②写手预检新增声线自检（继承 story-long-write 逐章对话声线自检，含"台词互换穿帮测试"）；③N4 语义审查新增声口核对维度（逐角色抽检+互换测试，单角色走音或系统性趋同均为 blocking）；④N6 阶段审查新增跨章声口趋同扫描（防"所有人越来越像同一个人说话"的 AI 长写病）；⑤初始化 Step 1 强制核验人物卡声口三件套。
